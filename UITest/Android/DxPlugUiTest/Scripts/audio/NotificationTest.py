...
class NotificationTest(unittest.TestCase):
    ...
    def jump_normal_articel_check(self,article):
        step = 1
    ...
    def testFinish(self):
        self.case_id = get_case(__file__)
        self.result = self.finish_check(AUDIO_ARTICLE)

    def testJump(self):
        self.case_id = get_case(__file__)
        self.result = self.jump_check(AUDIO_ARTICLE)

    def testJumpNormalArticle(self):
        self.case_id = get_case(__file__)
        self.result = self.jump_normal_articel_check(NORMAL_AUDIO_ARTICLE)
