...
class DbLib(DxDb):
    ...
    def get_update_info_by_db(self,ak=AK):
        self.connect()
        sql_str = 'select version,forcedver,content from vermgr where ak= \'%s\' and os = \'a\'' % (str(ak))
        r = self.do(sql_str)
        self.disconn()
        return r[0]

    def set_update_info_by_db(self,version,forcedver,content,ak=AK):
        self.connect()
        sql_str = 'update vermgr set version=\'%s\',forcedver=\'%s\',content=\'%s\' from vermgr where ak= \'%s\' and os = \'a\'' % (str(version),str(forcedver),str(content),str(ak))
        r = self.do(sql_str)
        self.disconn()
        return True
