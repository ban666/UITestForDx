...
class ChnlRequest(RequestForDx):
    ...
    def __init__(self,mode,ec='420000000010000',cc='420106',p='a',v='3.1.2',ak='1',dc = "1,0,0,93JhERG1UYLiW.p6UfdubuLUFveUZCjg6v.QMEkHKu8=#cc36a2ebfc32b210b90f91f8885de0bc",tid='a_imei865479020303095',pid = '100d855909470b42cd9',loc = '',chnl_para='cnhubei'):
        super(ChnlRequest,self).__init__(ec,cc,p,v,ak,dc,tid,pid,loc,chnl_para)
        self.mode = mode
        self.dx_mode = 'mcp/dx'
    ...
    def get_article(self, article, id='', cmd='nearer', psize='20',*args):
        ...
        if self.mode == self.dx_plug_mode:
            dx_method['22'] = [self.mode+u'/1/news/audio',para_id[self.mode]+'='+str(article['infoid'])]
        ...
        dx_ret = {
            ...
            '22':{self.dx_plug_mode:['data','audio']}
        }
    ...
    def get_head(self,id,*args):
        data = [self.mode+u'/1/head/get','id='+str(id)]
        for arg in args:
            data.append(arg)
        result = self.send_request_for_dx(*data)
        assert result['code']=='0'
    ...
    def get_top(self,id,psize='20'):
        data = [self.mode+u'/1/comm/top','id='+str(id),'psize='+str(psize)]
        result = self.send_request_for_dx(*data)
        assert result['code']=='0'
        return result['data']['list']
    ...
    def get_first_chnl_article_by_model(self,model):
        rechid = self.get_first_chnl()
        article_list = self.get_article_by_model(model,rechid)
        if len(article_list) >0:
            return article_list[0]
        return False

    def get_first_chnl(self,*args):
        chnl_id = False
        if self.mode == self.dx_mode:
            chnl_id = self.chnl()['sub'][0]['id']
        elif self.mode == self.dx_plug_mode:
            chnl_id = self.chnl()['list'][0]['id']
        return chnl_id
    ...
    def send_comment_by_name(self,name,comment,comid='',*args):
        db = DbLib()
        infoid = db.get_infoid_by_article_name(name)
        return self.send_comment(infoid,comment,comid,*args)

if __name__ == '__main__':
    api = ChnlRequest('mcp/plug/app')
