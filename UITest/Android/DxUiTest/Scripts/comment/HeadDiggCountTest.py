...
class HeadDiggCountTest(unittest.TestCase):
    ...
    def common_check(self):
        ...
        article_count = self.driver.find_element_by_id(HEAD_DIGG_COUNT).text
        assert article_count == str(0)
        print u'Step %s:上头条列表页点赞数为0时显示测试：OK' % (str(step))
        step+=1
        ...
        print u'Step %s:上头条列表页点赞数为<=99999时显示测试：OK' % (str(step))
        step+=1
        ...
        print u'Step %s:上头条列表页点赞数大于等于10万时显示测试：OK' % (str(step))
        step+=1
