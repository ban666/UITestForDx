...
class SubTypeTest(unittest.TestCase):
    ...
    def common_check(self):
        ...
        subs = self.driver.find_elements_by_class_name("android.widget.RadioButton")
        nums =  len(subs)-4
        sub_list = []
        for num in range(1,nums+1,1):
            sub_id = SUBTYPE + str(num)
            sub_id_el = self.driver.find_element_by_id(sub_id)
            sub_list.append(sub_id_el)
            print sub_id_el.text
            if num == 1:
                assert get_checked(sub_id_el) == True
            else:
                assert get_checked(sub_id_el) == False
    ...
    def name_check(self):
        ...
        for i in range(len(sub_list)):
            print sub_list[i].text,subtype_ret[i]
            assert sub_list[i].text == subtype_ret[i]
