...
def get_clue_digg_count(driver):
    return driver.find_element_by_id(CLUE_LIST_DIGG_COUNT).text


def get_clue_comment_count(driver):
    return driver.find_element_by_id(CLUE_LIST_COMMENT_COUNT).text


def digg_clue(driver):
    driver.find_element_by_id(CLUE_LIST_DIGG_BUTTON).click()
    return True


def send_txt_clue(driver,subtype,content,anony=0,loc='',pic=''):
    """
    :param driver:
    :param subtype: 0,1,2,3 分别对应4个频道
    :param content:
    :param anony:
    :param loc:
    :return:
    """
    driver.find_element_by_id(SEND_CLUE_TYPE_CHOOSE).click()
    driver.find_elements_by_id(SEND_CLUE_TYPE_CHECK)[int(subtype)].click()
    driver.find_element_by_id(SEND_CLUE_DESC_INPUT).send_keys(content)
    if loc != '':
        driver.find_element_by_id(SEND_CLUE_LOCATION).click()
        driver.find_element_by_id(LOCATION_EDIT_TEXT).send_keys(loc)
        driver.find_element_by_id(LOCATION_EDIT_OK).click()
    if anony!=0:
        driver.find_element_by_id(SEND_CLUE_ANONYMOUS).click()
    if pic != '':
        driver.find_elements_by_id(SEND_CLUE_PIC)[-1].click()
        sleep(WAIT_TIME)
        driver.find_elements_by_id(SEND_CLUE_CHECK_MARK)[0].click()
        driver.find_element_by_id(SEND_CLUE_PIC_CONFIRM).click()
        sleep(WAIT_TIME)
    driver.find_element_by_id(SEND_CLUE_CONFIRM_BUTTON).click()
    sleep(WAIT_TIME)
    return True
...
def send_comment(driver,content,in_page=True):
    ...
    try:
        ...
        pass
    try:
        driver.find_element_by_id(CLUE_LIST_COMMENT_BUTTON).click()
...
def get_to_clue_detail_by_search(driver,content):
    try:
        content = content.decode('utf-8')
    except:
        pass
    go_to_clue(driver)
    sleep(WAIT_TIME)
    driver.find_element_by_id(CLUE_SEARCH).click()
    driver.find_element_by_id(CLUE_SEARCH_EDIT).send_keys(content)
    driver.press_keycode(66)
    sleep(WAIT_TIME)
    driver.find_element_by_id(CLUE_LIST_DESC).click()
    sleep(WAIT_TIME)
    return True