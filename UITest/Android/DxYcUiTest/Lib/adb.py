...
def get_config_by_adb(package = desired_caps['appPackage']):
...
def get_userinfo_by_adb(package = desired_caps['appPackage']):
    cmd = 'adb shell cat '+ '/data/data/'+package+'/shared_prefs/userinfo.xml'
    print cmd
    p = os.popen(cmd)
    content = p.read()
    #print content
    if content.find('No such file or directory') != -1:
        print 'No such file'
        return False
    root = etree.XML(content)
    children = root.getchildren()
...
def del_ignore_version_by_adb(package = desired_caps['appPackage']):
    cmd = 'adb shell rm -rf '+ '/data/data/'+package+'/shared_prefs/version_share.xml'
    print cmd
    p = os.popen(cmd)
    content = p.read()
    #print content
...
def clear_cache_by_adb(package = desired_caps['appPackage']):
    cmd = 'adb shell rm -rf '+ '/data/data/'+package+'/cache'
    print cmd
    p = os.popen(cmd)
    content = p.read()
    #print content
...
def check_login_status(package = desired_caps['appPackage']):
    content = get_userinfo_by_adb(package)
    if not content:
        return False
    if content.get('uid'):
        return True
...
def download_and_install(url= 'http://10.99.113.28/ycdxapk/',package =desired_caps['appPackage'] ):
    apk = download_latest_apk(url)
    print apk
    uninstall(package)
    if install(apk):
        return True
...
def pull_file(path,save_path=IMAGE_PATH):
    fname = os.path.split(path)
    cmd = 'adb pull '+path+ ' '+save_path+fname[1]
    p = os.popen(cmd)
    content = p.read()
    #print content
    return save_path+fname[1]


def push_file(push_file,path):
    cmd = 'adb push '+push_file+ ' '+path
    p = os.popen(cmd)
    content = p.read()
    if content.find('100%')!=-1:
        return True
    return False

def push_picture(p_file,path):
    push_file(p_file,path)
    broad = 'adb shell am broadcast -a android.intent.action.MEDIA_SCANNER_SCAN_FILE -d file://'+path
    print broad
    p = os.popen(broad)
    return True
...
if __name__ == '__main__':
    ...
    # ret =  download_and_install('http://10.99.113.28/qjapk/')
    # print ret
    # print get_product_name()
    path = '/sdcard/Pictures/1.jpg'
    push = IMAGE_PATH+'/'+IMAGES.get('image1')
