...
def slide_up(driver,per=1,end = 1):
    x = driver.get_window_size()['width']
    y = driver.get_window_size()['height']
    driver.swipe(x/2, y/10*(10-per), x/2, y/10*end, 0)
...
def slide_down(driver,per=3,wait_time = 3):
    x = driver.get_window_size()['width']
    y = driver.get_window_size()['height']
    driver.swipe(x/2, y/10*2, x/2, y/10*(2+per), 0)
    sleep(WAIT_TIME)
...
def slide_right(driver,per=1):
    x = driver.get_window_size()['width']
    y = driver.get_window_size()['height']
    driver.swipe(x/4*0.5, y/2, x*(1+per)/4-1, y/2, 0)
...
def start_to_index(driver,mode,method='',cancel=True):
    ...
    try:
        ...
        while element_exsist(driver,'id',MY_COMM):
            if retry >= 10:
                print 'Error!'
                return False
            driver.find_element_by_id(INFORMATION).click()
            retry +=1
        return True
    except Exception,e:
        print sys._getframe().f_code.co_name
...
def cnv_request_caculator(driver,pic_width,pic_height):
    screen_width = driver.get_window_size()['width']
    screen_height = driver.get_window_size()['height']
    h0 = screen_width*7/15
    w1 = pic_width*h0/pic_height
...
def clear_notification(driver):
    ...
    if element_exsist(driver,'id',XIAOMI_SETTINGS):
        slide_right(driver)
    sleep(WAIT_TIME)
    notification_clear_el = NOTIFICATION_CLEAR_BUTTON.get('default') if not NOTIFICATION_CLEAR_BUTTON.has_key(get_product_name()) \
        else NOTIFICATION_CLEAR_BUTTON.get(get_product_name())
    try:
        driver.find_element_by_id(notification_clear_el).click()
    except:
        print 'no el find'
...
def is_login(driver):
    ...
    if IS_ROOT == True:
        ...
        driver.find_element_by_id(MY_CONFIG).click()
        if element_exsist(driver,'id',WELCOME_MSG) == False:
            result = True
    sleep(WAIT_TIME)
    return result
