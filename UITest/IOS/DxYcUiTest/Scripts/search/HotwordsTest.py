...
class HotwordsTest(unittest.TestCase):
    ...
    def common_check(self):
        ...
        hotwords = self.driver.find_element(*HOTWORDS_GROUP).find_elements_by_class_name('UIAButton')
        sleep(WAIT_TIME)
        hotwords[0].click()
        sleep(WAIT_TIME)
        #assert element_exsist(self.driver,'id',NORMAL_TITLE) or element_exsist(self.driver,'id',SEARCH_EMPTY_TIPS)
        assert element_exsist(self.driver,*SEARCH_EDITTEXT_CLEAR)
        assert element_exsist(self.driver,*HOTWORDS_GROUP) == False
