...
def get_push_state(driver):
    return driver.find_element(*PUSH_OPTION).get_attribute('checked')


@exception_handler
def set_push_state(driver,state):
    if get_push_state(driver) != state:
        driver.find_element(*PUSH_OPTION).click()
    return True
...
def set_location(driver,location,retry=3,wait=3):
    ...
    if get_location(driver) == location:
        driver.find_element(*LOCATION_SETIINGS).click()
        sleep(wait)
        back(driver)
        return True
    driver.find_element(*LOCATION_SETIINGS).click()
...
def get_location(driver):
    return driver.find_element(*LOCATION_SETIINGS).text


@exception_handler
def get_version_code(driver):
    return driver.find_element(*VERSIONS).text


def go_to_suggestion(driver):
    driver.find_element(*SUGGESTION).click()
    return True

def clear_cache(driver):
    driver.find_element(*CACHE_CLEAR).click()
    driver.find_element(*CACHE_CLEAR_CONFIRM).click()
    return True