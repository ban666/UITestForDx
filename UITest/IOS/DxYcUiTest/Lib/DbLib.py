...
class DbLib(DxDb):
    ...
    def delete_user_by_phone(self,phone):
        self.connect()
        self.do('delete from member where phone = %s' % (str(phone)))
        self.disconn()
    ...
    def clear_comment_by_id(self,infoid):
        self.connect()
        sql_str = 'delete comment from comment where infoid = %s'% (infoid)
        r = self.do(sql_str)
        self.disconn()
        return True
    ...
    def get_subtype_by_db(self,z='420500'):
        self.connect()
        sql_str = 'select clcaid,name from cluecate where z=%s ORDER by clcaid' % (str(z))
        #print sql_str
        r = self.do(sql_str)
        self.disconn()
        return r

    def set_update_version(self,version,force_ver='',ak=AK,os='i'):
        self.connect()
        sql_str = 'update vermgr set version=\'%s\',forcedver=\'%s\' where ak=%s and os=\'%s\'' \
                  % (str(version),str(force_ver),str(ak),str(os))
        #print sql_str
        r = self.do(sql_str)
        self.disconn()
        return True
    ...
    def get_update_version(self,ak=AK,os='a'):
        self.connect()
        sql_str = 'select version,forcedver,content from vermgr where ak=%s and os=\'%s\'' \
                  % (str(ak),str(os))
        #print sql_str
        r = self.do(sql_str)
        self.disconn()
    ...
    def set_subtype_name_by_id(self,name,sid):
        try:
            name = name.encode('utf-8')
        except:
            pass
        self.connect()
        sql_str = 'update cluecate set name=\'%s\' where clcaid=%s' % (str(name),str(sid))
        #print sql_str
        r = self.do(sql_str)
        self.disconn()
        return True
    ...
    def get_latest_suggestion_info(self):
        self.connect()
        sql_str = 'select mid,content,z,version from suggest ORDER by createTime desc limit 1'
        #print sql_str
        r = self.do(sql_str)
        self.disconn()
        return r[0]

    def get_clue_pic_info_by_db(self,content,path = '/wwwdata/mcp/resource/tipoff/'):
        try:
            content = content.encode('utf-8')
        except:
            pass
        self.connect()
        sql_str = 'select t2.comThumb from clue t1,information t2 where t1.content = \'%s\' ' \
                  'and t1.infoid = t2.infoid ORDER by t2.createTime' % (content)
        #print sql_str
        r = self.do(sql_str)
        self.disconn()
        if len(r)>0:

            pic = eval(r[0].get('comThumb'))[0].get('u').replace('${userTipoffPath}',path)
            return pic
        return False

    def get_latest_comid_by_article(self,article_name):
        try:
            article_name = article_name.encode('utf-8')
        except:
            pass
        self.connect()
        sql_str = 'select t1.comid from comment t1,information t2 where t1.infoid = t2.infoid ' \
                  'and t2.title = \'%s\' ORDER by t1.createTime desc' % (article_name)
        #   print sql_str
        r = self.do(sql_str)
        self.disconn()
        return r[0]['comid']
if __name__ == '__main__':
    a = DbLib()
    from datetime import datetime
    t = a.get_subtype_by_db()[0]['clcaid']
    print t
    #print a.get_clue_pic_info_by_db('单方图')
    #print a.change_play_count_by_name('自动化音频新闻',22)
    #print a.get_play_count_by_name('自动化音频新闻')
